RampClassifierTraining=function(Data,Cls,HiddenNeurons,Epochs=1000000,PreTrainedNN,PlotIt=F){
  # ein Klassifikator mit der Ramp-funktion 
  # V  = RampClassifierTraining(Data,Cls,HiddenNeurons,Epochs)
  ...
  ramp <- function(x){if(x < 0){return(0);}else{return(x);}}
  
  
  if(missing(PreTrainedNN))
    nn <- neuralnet( form , dat, hidden= HiddenNeurons, stepmax=Epochs, act.fct = ramp, lifesign = "full")
  else
    nn <- neuralnet( form , dat, hidden= HiddenNeurons, stepmax=Epochs, act.fct = ramp, startweights=PreTrainedNN, lifesign = "full")
