CentroidClassifier <- function (data, cls, testData = data, centroidMethod = "mean") {
...
for (i in 1:nrOfData) {
nearestInd[i] <- BestBuddy( testData[i, ], centroid)[[1]]
assignedCls[i] <- uniqueClasses[nearestInd[i]]
}

return(list( assignedCls = assignedCls, uniqueClasses = uniqueClasses, centroid = centroid) )
}