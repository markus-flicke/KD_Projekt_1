ClassPDEplot <- function(Data,Cls,ColorSequence= DefaultColorSequence(),ColorSymbSequence = DefaultColorSymbSequence(),PlotLegend =1,SameKernelsAndRadius=0,xlim,ylim,...){
  ...
  if(SameKernelsAndRadius ==1){ # gleiche Kernels und ParetoRadius benutzen fuer alle Verteilungen
    ...
    UniqueClasses <- CUC$UniqueClasses
    ...
    ClassParetoDensities = matrix(Kernels,ncol=1) %*% ones(1,NrOfClasses);
    Weight=zeros(length(UniqueClasses),1)
    ...
    figure()
    for(c in 1:NrOfClasses){
      plot(Kernels,ClassParetoDensities[,c]*rep(Weight[c],nrow(ClassParetoDensities)),Ylimes=c(0,Ylimes),xlim=xlim,typ='l',col=BMClassPlotSymbols[c,],main="ParetoDensityEstimation(PDE)", xlab='Data', ylab='PDE',xaxs='i',yaxs='i')
      par(new=TRUE); 
    }
    ...
    for(c in 1:NrOfClasses){
      if(missing(ylim))
        plot(kernels[[c]],paretoDensity[[c]],ylim=c(0,Ylimes),xlim=xlim,typ='l',col=BMClassPlotSymbols[c],main="ParetoDensityEstimation(PDE)", xlab='Data', ylab='PDE',xaxs='i',yaxs='i',...)
      else
        plot(kernels[[c]],paretoDensity[[c]],ylim=ylim,xlim=xlim,typ='l',col=BMClassPlotSymbols[c],main="ParetoDensityEstimation(PDE)", xlab='Data', ylab='PDE',xaxs='i',yaxs='i',...)
  ...
  if(PlotLegend ==1){
    legend('topright', legend=UniqueClasses,fill=BMClassPlotSymbols)
  }
