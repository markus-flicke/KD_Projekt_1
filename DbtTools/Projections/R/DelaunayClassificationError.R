...
if(length(Cls) <3 )               stop('DelaunayClassifError: at least 3 points required')
...
for(i in 1:nrow(Data)){ #Setze 0, falls nicht direkt benachbahrt
  if(AnzNN[i]<nn){
...
return(list(DCE=DCE,DCEperPoint=DCEperPoint,nn=nn,AnzNN=AnzNN,NNdists=NNdists,HD=HD,IsInterDistance=IsInterDistance,DelaunayDists=DelaunayDists))
}    
